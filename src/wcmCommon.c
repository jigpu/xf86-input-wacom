 ...
 * @param wrap     Value after which wraparound occurs (0 if axis does not wrap)
 ...
 */
static int getScrollDelta(int current, int old, int wrap, Bool invert, Bool bitwise)
{
	int delta;
				 ...
				 int first_val, int num_vals, int *valuators)
{
	...
	delta = getScrollDelta(ds->abswheel, priv->oldWheel, MAX_PAD_RING, TRUE, FALSE);
	if (delta && IsPad(priv) && priv->oldProximity == ds->proximity)
	{
	...
	delta = getScrollDelta(ds->abswheel2, priv->oldWheel2, MAX_PAD_RING, TRUE, FALSE);
	if (delta && IsPad(priv) && priv->oldProximity == ds->proximity)
	{
